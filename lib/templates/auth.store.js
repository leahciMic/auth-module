...
function getAxios(ctx) {
  return ctx.$axios || ctx.app.$axios;
}

function getCtx(ctx) {
  return ctx.$ctx || ctx.app.$ctx;
}
...
export default {
  ...
  actions: {
    ...
    updateToken ({ commit }, token) {
      ...
      getAxios(this).setToken(token, '<%= options.token.type %>');
      ...
      if (process.browser) {
        ...
        getCtx(this).res.setHeader('Set-Cookie', Cookie.serialize('<%= options.token.cookieName %>', token, params))
      }
      <% } %>
    },
    <% } %>
    ...
    fetchToken ({ dispatch }) {
      ...
      if (!token) {
        const cookieStr = process.browser ? document.cookie : ( this.$ctx || this.app.$ctx).req.headers.cookie
    ...
    async fetch ({ getters, state, commit, dispatch }, { endpoint = '<%= options.user.endpoint %>' } = {}) {
      ...
      try {
        const data = await getAxios(this).$get(endpoint)
    ...
    async login ({ dispatch }, { fields, endpoint = '<%= options.login.endpoint %>' } = {}) {
      ...
      let data = await getAxios(this).$post(endpoint, fields)
    ...
    async logout ({ dispatch, state }, { endpoint = '<%= options.logout.endpoint %>' } = {}) {
      // Server side logout
      try {
        await getAxios(this).$<%= options.logout.method.toLowerCase() %>(endpoint)
      } catch (e) {
